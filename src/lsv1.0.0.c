...
int main(int argc, char const *argv[])
{
    ...
    /* parse -l option (we only accept -l for this feature) */
    while ((opt = getopt(argc, (char * const *)argv, "l")) != -1) {
        if (opt == 'l') longflag = 1;
    }

    if (optind == argc) {
        /* no non-option args: list current directory */
        if (longflag) do_ls_long(".");
        else do_ls(".");
    } else {
        for (int i = optind; i < argc; i++) {
            if (optind + 1 < argc) {
                /* print header when multiple dirs specified */
                printf("%s:\n", argv[i]);
            } else if (argc - optind > 1) {
                printf("Directory listing of %s :\n", argv[i]);
            }
            if (longflag) do_ls_long(argv[i]);
            else do_ls(argv[i]);
            if (i + 1 < argc) puts("");
        }
    }
